...
agent ReactiveControl
{
	...
	behaviour followRightWall priority 1.0
	{
	    fusion turn, speed;
	    ...
	    rules
	    {
	        ...
	        // Mantener distancia con la pared derecha
	        if (rightd is CLOSE)                     turn is TL;
	        if (rightd is NEAR)                      turn is TSL;
	        if (rightd is MED)                       turn is TC;
	
	        // Manejar esquinas y giros cerrados
	        if ((front is CLOSE) && (rightd is FAR)) turn is TL;
	        if ((front is CLOSE) && (rightd is CLOSE)) turn is TL;
	
	        // Reducir velocidad en esquinas
	        if ((front is CLOSE) || (rightd is CLOSE)) speed is SLOW;
	    }
	}

	behaviour recoverRightWall priority 1.0
	{
	    fusion turn, speed;
	    ...
	    rules
	    {
	        ...
	        background (0.01) turn is TSL; 
	
			// Gira a la derecha para buscar la pared
	        if (rightd is FAR)                       turn is TSR;
	        if (rightd is VFAR)                      turn is TR;
	        
	        // Reducir velocidad y recolocar cerca de la pared
	        if (front is CLOSE) 					turn is TSL, speed is SLOW;
	    }
	}
	...
	blender
	{
		...
		rules
		{
			// Valores predeterminados
 			background (0.01) 					followRightWall is LOW;
        	background (0.01) 					recoverRightWall is LOW;

			// Mantener la pared derecha
			if (rightd is CLOSE)				followRightWall is HALFH;
        	if (rightd is NEAR)                 followRightWall is HIGH;
        	if (rightd is MED)                  followRightWall is HALFL;
        	
        	// Recuperar la pared derecha
        	if (rightd is FAR)  				recoverRightWall is HIGH;
        	if (rightd is VFAR)					recoverRightWall is HIGH;
